...
export default function Lanyard({
  ...
  return (
    <div className="relative z-0 flex h-full w-full origin-center items-center justify-center transform scale-100">
      <Canvas
        camera={{
          position,
          fov,
...
function Band({
  ...
  const texture = useTexture(
    lanyardImage || LANYARD_TEXTURE_PATH
  );
  ...
  const cardMap = useMemo(() => {
    ...
    const qualityScale = isMobile ? 2 : 3;

    const W = Math.max(
      1024,
      Math.floor(baseImg.width * qualityScale)
    );

    const H = Math.max(
      1024,
      Math.floor(baseImg.height * qualityScale)
    );
